...
<script>
	...
	function run_pagination(page){ // pagination function
		...
		$.post(
			'<?php echo base_url(); ?>index.php/Director/list_Class/'+page, 
	...
	function add_Class(){ // clear and opens modal
		$('.modal-title h5').html("Add Class Information")
		$('#id').val('');
		$('#cmd-type').val('add');
		$('#ay').val('');
		$('#sec').val('');
		$('#cmd-subj').html('<option value="" disabled="disabled">Choose Subject</option>');
		$('#cmd-fa').html('<option value="" disabled="disabled">Choose Faculty</option>');
		$('#modal1').openModal();
	}
	function Update_class(id){ // get Data from the database and opens the modal Update
		var dataid = 'id='+ id;
		var fa_id;
		var subj_id;
		$.ajax({
			url: "<?php echo base_url(); ?>index.php/Director/View_individual_class",
			type:'POST',
			data: dataid,
			dataType:'json',
			beforeSend: function(){
			...
			success: function(data){
				...
				cbo_change(data['course_id'], subj_id, fa_id);
				...
				$('#cmd-course').val(data['course_id']);
				$('#combo-for-course .select-wrapper input').val(data['course_name']);
				
				$('.modal-title h5').html("Edit Class Information")
	...
	function Delete_class(id){
		...
		if(c){
			$.ajax({
				url:'<?php echo base_url(); ?>index.php/Director/Deactivate_Class',
				type:'POST',
				data: 'id='+id,
				beforeSend: function(){},
				success: function(data){
	...
	function search_Class(){ // Search
		...
		var crs = ($('#course').val() == null ? "" : $('#course').val());
		$.ajax({
			url: "<?php echo base_url(); ?>index.php/Director/list_Class",
